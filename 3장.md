...
첫째, **CLIP(Radford et al., 2021)**을 활용한 제로샷(Zero-Shot) 분류를 통해 학습 데이터가 없는 클래스에 대한 사전 예측을 수행한다.
둘째, **Prototypical Networks(Snell et al., 2017)** 기반 퓨샷(Few-Shot) 학습을 적용하여, 클래스당 5~10개의 소량의 라벨로 Tail 클래스의 시각적 특징을 학습한다.
...
Tail 클래스는 희소하고, Head 클래스는 과대표집되어 있다. 기존 지도학습 모델(YOLOv8 등)은 Head 클래스에서는 90 % 이상의 mAP를 보이지만, Tail 클래스에서는 40 % 미만의 낮은 성능을 보인다 (Jocher et al., 2023).
...
CLIP (Radford et al., 2021)은 4억 개의 이미지–텍스트 쌍으로 학습된 모델로, 이미지 인코더 (f_{\text{img}})와 텍스트 인코더 (f_{\text{text}})가 공통 임베딩 공간에서 정렬(alignment)되도록 훈련되어 있다.
...
최근 연구에서도 이러한 제로샷 탐지 접근법이 다양한 도메인에서 확장 가능함이 보고되었다 (Bansal et al., 2018; Gupta et al., 2022).

## 3.3 퓨샷 학습 (Few-Shot Learning)

### (1) 개요

퓨샷 학습은 클래스당 소수의 라벨링 샘플만으로도 학습 가능한 메타 학습(Meta-Learning) 접근이다 (Vinyals et al., 2016).
본 연구에서는 **Prototypical Networks**를 채택하여, Tail 클래스의 K개 샘플로부터 대표 프로토타입을 구성하고, 새로운 이미지의 거리를 측정하여 분류한다.

### (2) 임베딩 네트워크 구조

* **백본(Backbone):** ResNet-50 (He et al., 2016)
* **출력 임베딩 차원:** 1600
* **공간 맥락 보존:** Spatial Pyramid Pooling(SPP) 모듈을 추가하여 객체 위치 정보를 유지 (Zhou et al., 2016).
...
매 에피소드마다 새로운 클래스 조합을 샘플링하여 메타 일반화 능력을 강화한다 (Finn et al., 2017).
...
Bansal, A., Sikka, K., Sharma, G., Chellappa, R., & Divakaran, A. (2018). Zero-Shot Object Detection. *arXiv preprint arXiv:1804.04340.*

Cao, K., Wei, C., Gaidon, A., Arechiga, N., & Ma, T. (2019). Learning Imbalanced Datasets with Label-Distribution-Aware Margin Loss. *Advances in Neural Information Processing Systems (NeurIPS 2019),* 1567–1578.

Finn, C., Abbeel, P., & Levine, S. (2017). Model-Agnostic Meta-Learning for Fast Adaptation of Deep Networks. *Proceedings of the International Conference on Machine Learning (ICML 2017),* 1126–1135.

Gupta, A., Narayan, S., & Bhowmick, S. (2022). CLIP for Domain-Adaptive Zero-Shot Object Detection. *IEEE Access,* 10, 121045–121058.
...
Radford, A., Kim, J. W., Hallacy, C., Ramesh, A., Goh, G., Agarwal, S., et al. (2021). Learning Transferable Visual Models from Natural Language Supervision. *Proceedings of the International Conference on Machine Learning (ICML 2021),* 8748–8763.

Ren, J., Yu, C., Sheng, S., Ma, X., Zhao, H., Yi, S., & Li, H. (2021). Balanced Meta-Softmax for Long-Tailed Visual Recognition. *NeurIPS 2021,* 4175–4186.
